...
GitHub 是唯一官方源；Gitee 仅作为国内只读镜像，SkillHub 作为发现和安装入口。当前镜像地址为 `https://gitee.com/songhai-dg/review-write`。不要在 Gitee 单独开发，也不要把 SkillHub 下载包当作新的源码源。

仓库维护者可在 GitHub 仓库的 Actions secrets 中配置 `GITEE_USERNAME` 和 `GITEE_TOKEN`。之后 `.github/workflows/mirror-gitee.yml` 会在 `main` 或 `v*.*.*` 标签更新时，自动推送对应分支或标签；它不会强制覆盖 Gitee，也不会读取用户正文或运行时凭证。Release 中的 `.skill` 和 `.sha256` 仍以 GitHub 为校验源，上架 SkillHub 时应使用同一份附件。
...
GitHub is the single canonical source. Gitee is a read-only domestic mirror, and SkillHub is a discovery and installation channel. The current mirror is `https://gitee.com/songhai-dg/review-write`. Do not develop a second copy on Gitee or treat a SkillHub download as a new source repository.

Repository maintainers can configure `GITEE_USERNAME` and `GITEE_TOKEN` as GitHub Actions secrets. `.github/workflows/mirror-gitee.yml` then pushes `main` and `v*.*.*` tags to Gitee without force-overwriting the mirror. GitHub remains the verification source for Release `.skill` and `.sha256` assets; SkillHub listings should use those same assets.
